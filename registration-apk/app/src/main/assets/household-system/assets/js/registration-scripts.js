...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const getEffectiveUserId = () => {
    ...
    const access = window.CabarianOfflineAccess?.read?.();
    return String(access?.userId || "").trim();
  };

  const getOfflineStaffAuth = () => {
    const access = window.CabarianOfflineAccess?.read?.();
    return access && access.username && access.reauthToken ? access : null;
  };

  const AUTO_REAUTH_ENDPOINT = "auto-reauth-api.php";
  ...
  const headEducationSelect = document.getElementById("head_education") || (censusForm ? censusForm.querySelector('select[name="education"]') : null);
  const headEducationSubfields = Array.from(document.querySelectorAll(".head-education-subfield"));
  ...
  const syncConnectivityState = async ({ force = false } = {}) => {
    ...
    const probePromise = (async () => {
      let reachable = false;
      try {
        let response = await fetch(`${PRESENCE_ENDPOINT}?_=${now}`, {
          method: "GET",
          credentials: "same-origin",
          cache: "no-store",
  ...
  const serializeHeadData = () => {
    ...
    if (String(data.education || "").trim().toLowerCase() === "no formal education") {
      data.degree = "";
      data.school_name = "";
      data.school_type = "";
      data.dropout = "No";
      data.osy = "No";
      data.currently_studying = "No";
    }
    return data;
  };
  ...
  const updateHeadEducationVisibility = () => {
    const educationSelectEl = headEducationSelect || (censusForm ? censusForm.querySelector('select[name="education"]') : null);
    if (!educationSelectEl) return;
    const isNoFormal = String(educationSelectEl.value || "").trim().toLowerCase() === "no formal education";
    const subfields = headEducationSubfields.length > 0 ? headEducationSubfields : Array.from(document.querySelectorAll(".head-education-subfield"));
    subfields.forEach((field) => {
      field.style.display = isNoFormal ? "none" : "";
    });
    if (isNoFormal && censusForm) {
      const degreeInput = censusForm.querySelector('input[name="degree"]');
      const schoolNameInput = censusForm.querySelector('input[name="school_name"]');
      const schoolTypeSelect = censusForm.querySelector('select[name="school_type"]');
      const dropoutSelect = censusForm.querySelector('select[name="dropout"]');
      const osySelect = censusForm.querySelector('select[name="osy"]');
      const studyingSelect = censusForm.querySelector('select[name="currently_studying"]');
  ...
  const syncRecordToServer = async (record) => {
    ...
    if (!csrfToken || csrfToken === "offline-token") {
      try {
        const presResp = await fetch("auth-presence.php", { cache: "no-store", credentials: "same-origin" });
        const presData = await presResp.json();
        if (presData && presData.csrf_token) {
          csrfToken = String(presData.csrf_token).trim();
        } else {
          await autoReauthenticateOnline();
        }
      } catch {
        await autoReauthenticateOnline();
      }
    ...
    let { response, payload } = await executeUpsert();

    // If server session expired or unauthorized, attempt background re-auth and retry once
    if (!response.ok && (response.status === 401 || response.status === 403)) {
      const reauthenticated = await autoReauthenticateOnline();
      if (reauthenticated) {
        const retryResult = await executeUpsert();
  ...
  const renderPendingSyncModal = () => {
    ...
    const queue = getSyncQueue();
    const pendingSyncNowBtn = document.getElementById("pendingSyncNowBtn");
    if (pendingSyncTitleCount) {
      pendingSyncTitleCount.textContent = `(${queue.length})`;
    }
    if (pendingSyncNowBtn) {
      pendingSyncNowBtn.disabled = !queue.length || pendingActionBusy;
    }
    if (!queue.length) {
  ...
  const renderPreview = async () => {
    ...
    const sections = [
      ...
      buildSection("C. Education", String(valueOf("education")).trim().toLowerCase() === "no formal education" ? [
  ...
  const headEduEl = headEducationSelect || (censusForm ? censusForm.querySelector('select[name="education"]') : null);
  if (headEduEl) {
    headEduEl.addEventListener("input", () => {
      updateHeadEducationVisibility();
      saveHeadData();
    });
    headEduEl.addEventListener("change", () => {
      updateHeadEducationVisibility();
      saveHeadData();
    });
  }
  ...
  const renderMemberModal = (member) => {
    ...
    const rows = [
      ...
      ...(String(member.education || "").trim().toLowerCase() === "no formal education" ? [] : [
  ...
  const triggerAutoSyncWhenOnline = async ({ delayMs = 0, forceProbe = true } = {}) => {
    if (autoSyncTriggerTimer) {
      window.clearTimeout(autoSyncTriggerTimer);
      autoSyncTriggerTimer = null;
    }
    const run = async () => {
      clearSyncSuccessState();
      serverReachable = null;
      ...
      await autoReauthenticateOnline();
      await syncConnectivityState({ force: forceProbe });
      updateSyncStatus();
      updateLoadExistingButtonVisibility();
      updateOfflineUIState();
      if (isAppOnline()) {
    ...
    if (delayMs > 0) {
      autoSyncTriggerTimer = window.setTimeout(run, delayMs);
    } else {
      await run();
    }
  };
  ...
  document.addEventListener("visibilitychange", () => {
    ...
    void triggerAutoSyncWhenOnline({ delayMs: 250, forceProbe: true });
  });
  ...
  const pendingSyncNowBtn = document.getElementById("pendingSyncNowBtn");
  if (pendingSyncNowBtn) {
    pendingSyncNowBtn.addEventListener("click", async () => {
      pendingSyncNowBtn.disabled = true;
      try {
        await autoReauthenticateOnline();
        await syncConnectivityState({ force: true });
        updateSyncStatus();
        ...
        pendingSyncNowBtn.disabled = false;
      }
    });
  }
